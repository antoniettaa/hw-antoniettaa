...
```{r}
select(gapminder, c("year", "lifeExp", "country")) # c works to make list 
...
```{r}
gapminder %>%
  filter(continent == "Asia" , pop > 100000000) # use parenthesis so it takes that value and not an object
...
```{r}
gapminder %>%
  filter(country %in% c ("Brazi" , "China"), pop > 100000000) # %in% is like == but it checks your value against all other values in the vector
```
...
gapminder %>%
  ...
  mutate(gdpBill = round(gdpPercap , digits = 2))
```
...
psych::bfi %>%
  ...
  mutate(across(A1:O5, ~ (.x - 1) / 5 * 100)) ## across is used when there is a lot of variables so it does the same across all those 
# tilda is telling it to do the operations , and .x is all the variables 
```
...
```{r}
gapminder %>% 
  filter(country == "Canada") # calling columns by name 
...
gapminder %>% 
  summarize(mu = mean(lifeExp),
            sigma = sd(lifeExp))  # summarize is how to reduce a data frame 
```
...
rowwise() #treats each variable like its own group
```
...
# ungroup if you are doing rowwise and then mutate, so it goes back to working with the entire data set 
```
